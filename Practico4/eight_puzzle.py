...
class EightPuzzle(gymnasium.Env):
    ...
    def _make_puzzle(self):
        tiles = list(range(self.size**2))  # 0 is blank
    ...
    def step(self, action):
        ...
        for i, row in enumerate(self.state):
            for j, tile in enumerate(row):
                correct_row = np.floor(tile / self.size) - (
                    1 if tile % self.size == 0 else 0
                )
    ...
    def render(self, mode="human", close=False):
        ...
        if self.last_action is not None:
            outfile.write(
                "Action: {0}\n".format(["U", "R", "D", "L"][self.last_action])
            )
            outfile.write("\n")
        else:
            outfile.write("\n")
